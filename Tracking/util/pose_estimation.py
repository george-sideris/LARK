...
class pose_estimation:
    ...
    def __init__(self, framerate=75, plotting=False, aruco_dict=cv2.aruco.DICT_ARUCO_MIP_36H12, LMcriteria=(cv2.TERM_CRITERIA_EPS+cv2.TERM_CRITERIA_COUNT, 20, 1.19209e-07)):
        # OpenCV Stuff
    ...
    def estimate_pose_board(self,reference_board:cv2.aruco.Board, target_board:cv2.aruco.Board, corners, ids, cameraMatrix, distCoeffs=np.array([[0., 0., 0., 0., 0.]])):
        ...
        if ids is not None and len(ids) > 0:
            ...
            ref_val, ref_rvec, ref_tvec = cv2.solvePnP(ref_obj_pts,ref_img_pts,cameraMatrix,distCoeffs,
                                            rvec=None,tvec=None,useExtrinsicGuess=False,flags=solve_flag)
            # ref_val, ref_rvec, ref_tvec, _ = cv2.solvePnPRansac(ref_obj_pts,ref_img_pts,cameraMatrix,None,
            #                                             rvec=None,tvec=None,useExtrinsicGuess=False,flags=solve_flag, reprojectionError=.4)
            # Pose of Target Board
            target_val, target_rvec, target_tvec = cv2.solvePnP(target_obj_pts,target_img_pts,cameraMatrix,distCoeffs,
                                                    rvec=None,tvec=None,useExtrinsicGuess=False,flags=solve_flag)
            # target_val, target_rvec, target_tvec, _ = cv2.solvePnPRansac(target_obj_pts,target_img_pts,cameraMatrix,None,
            #                                                 rvec=None,tvec=None,useExtrinsicGuess=False,flags=solve_flag, reprojectionError=.4)

...
def plot_r_norm_matrix():
    ...
    r_norm_matrix = np.column_stack(columns).T  # Transpose to put cameras on Y-axis
    num_cameras = r_norm_matrix.shape[0]  # Now cameras are rows
    num_positions = r_norm_matrix.shape[1]  # Now positions are columns
    fig_width = num_positions * 0.6
    fig_height = num_cameras * 0.6
    ...
    plt.xlabel('Position')
    plt.ylabel('Camera')
    plt.xticks(np.arange(num_positions), positions)
    plt.yticks(np.arange(num_cameras), camera_labels)
...
def plot_violin_norms():
    ...
    labels = [f'Camera {i+1}' for i in range(5)]
    if INCLUDE_MONO_AVERAGE:

...
class MarkerFactory:
    ...
    def create_marker(tile_size, marker_size, id, margin, diam_size, penta_rotation, arucoDict, alignment="centre"):
...
def main():
    TARGET = True  # True for tool/target, False for reference
    arucoDict = cv2.aruco.DICT_APRILTAG_16h5
    dictName = "16h5"  # Used for filename, change whenever arucoDict is changed
    marker_id = 0 if TARGET else 11  # id of first tag (0 for target, 11 for reference)
    marker_mm = 24 if TARGET else 33  # Marker side length in mm (first is for reference, second for target)
    penta_mm = 27 if TARGET else 39.5 # Pentagon side length in mm
    filePath = f'GeneratedMarkers/{"target" if TARGET else "reference"}_{marker_mm}_{penta_mm}_{dictName}.png'
    marker_in = marker_mm/25.4  # Marker side length in inches
    penta_in = penta_mm/25.4  # Pentagon side length in inches
    dpi = 400  # Page resolution in pixels per inch (72 if using cv2.imwrite)
    page_size = (8.5, 11)  # Page size in inches
    diam = 2 * (penta_in / 10 * np.sqrt(50 + 10*np.sqrt(5))) # Pentagon circumcircle diameter
    margin_in = 0.4 if TARGET else 0.01  # Page margins in inches
    ...
    for i in range(4):
        for j in range(3):
            ...
            marker_img = marker_factory.create_marker(tile_size, marker_size, marker_id, margin, diam_size, penta_rotation=angle, alignment="centre", arucoDict=arucoDict)
    ...
    RGBimage = cv2.cvtColor(full_page, cv2.COLOR_BGR2RGB)
